...
This action plan outlines the steps to install and configure Arch Linux on a Lenovo ThinkBook 14+ 2025 Intel Core Ultra 7 255H without dGPU but with Intel iGPU (Intel Arc 140T), **using GNOME Wayland, BTRFS, LUKS2, TPM2, AppArmor, systemd-boot with UKI, Secure Boot, and an OCuP4V2 OCuLink GPU Dock ReDriver with an AMD eGPU**. This laptop has two M.2 slots; we will install Windows in one slot to facilitate BIOS and firmware updates, at least initially.

**Observation:** We are not adopting the `linux-hardened` kernel due to complexity in the eGPU setup and performance penalties. Nevertheless, many security enhancements from the `linux-hardened` kernel are manually incorporated.

**Attention:** Before executing commands, especially those involving **dd, mkfs, cryptsetup, parted, and efibootmgr**, re-read them multiple times to ensure you understand their effects and that the target device/partition is correct. Verify that LUKS and TPM unlocking work perfectly before enabling Secure Boot, and confirm Secure Boot functions before configuring the eGPU.

## Step 1: Verify Hardware

Access UEFI BIOS (press F1 at boot):
...
Follow some of the installation privacy advice from the Privacy Guides Wiki on [Minimizing Windows 11 Data Collection](https://discuss.privacyguides.net/t/minimizing-windows-11-data-collection/28193).
...
- Verify NVMe drives in **Windows Disk Management**.

Review the guides for additional privacy in post-installation: [Group Policy](https://www.privacyguides.org/en/os/windows/group-policies/) and [Windows Privacy Settings](https://discuss.privacyguides.net/t/windows-privacy-settings/27333).

Before starting the next steps, back up registry settings (PowerShell):
- `reg export "HKLM\SOFTWARE" C:\backup_registry.reg`

Disable diagnostic data, feedback, and telemetry services (PowerShell):
...
Disable Cortana and web search in Start menu (PowerShell):
...
Uninstall preinstalled apps (e.g., Xbox, Candy Crush) (PowerShell):
...
Replace secure_boot_number with secure boot number, the command below should return 0:
...
Install Bubblejail:
- `paru -S --needed bubblejail`

**DEPRECATED:** Install Alacritty with SIXEL support from ayosec/alacritty latest version, double check if this remains the latest otherwise change v0.15.1-graphics:
- `git clone https://aur.archlinux.org/alacritty-sixel-git.git`
...
- Test if correct: `bubblejail run Alacritty -- env | grep -E 'WAYLAND|XDG_SESSION_TYPE'`

Install Astal:
- `paru -S astal-git ags-git`
...
# The envars below may be NOT INCLUDED and rely on switcheroo-control to automatically drive the use of the AMD eGPU or the Intel iGPU. DO NOT ADD INITIALLY:
...
Check if vfio and qemu needs to be signed:
...
Pacman Hook for Binary Verification - mimics fapolicyd’s package-based trust model (After updates, check /var/log/pacman.log for pacman -Qkk output. If altered files are detected, investigate with pacman -Qkk | grep -v '0 altered files'.)
...
- Store sensitive data (UEFI password, LUKS passphrase, keyfile location, MOK password) in Bitwarden.
- Document recovery steps in Bitwarden:
...
- Offsite Snapshots:
  - To be refined saving the data in local server - check btrbk and restic
...
### n) Remove after Installation

After completing the installation and verifying the system boots correctly, you can safely remove arch-install-scripts with:
...
### p) Astal Integrity Security Checker
...
### q) Windows Post-Install Hardening Guide

https://discuss.privacyguides.net/t/windows-post-install-hardening-guide/27335

## Step 19: User Customizations

a) Setup Adwaita (preferable) or Osaka Light as the GNOME Shell theme in gnome-tweaks. # Skip Osaka Light or Catppuccin as separate themes, replicating their aesthetics with CSS to align with Libadwaita.
